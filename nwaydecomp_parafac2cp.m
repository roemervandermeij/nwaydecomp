function [comp,P,ssqres,expvar,scaling,tuckcongr] = nwaydecomp_parafac2cp(dat,ncomp,specmodes,varargin)
...
%   specmodes   = vector with length equal to ndims(dat) with 1, 2 and 3 (one of the cross-product dimensions must be 2)
...
if (sum(specmodes==3)~=1) || (sum(specmodes==2)~=1) || (sum(specmodes==1)~=1) || isempty(specmodes) || (numel(specmodes)~=ndims(dat)) || (sum(specmodes)~=6)
  error('improper specmodes')
end

% throw error if dat is not 3-way
if ndims(dat)~=3
  error('input dat for parafac2cp must be 3-way')
end

% throw an error for non-present ssqdatnoncp
if isempty(ssqdatnoncp)
  error('sums of squares of original data should be present in ssqdatnoncp')
end
...
% datnoncp   = permute(datnoncp,permordorg);
...
smode = size(dat);
% smodeincomplnoncp = NaN(1,smode(estimmode));
% permorder    = [incomplmode estimmode utilitymodes];
% incompldat   = permute(datnoncp,permorder);
% incompldat   = incompldat(:,:); % select only the incomplmode and estimmode
% for iestimval = 1:smode(estimmode)
%   smodeincomplnoncp(iestimval) = sum(~isnan(incompldat(:,iestimval)));
% end
smodey = smode;
smodey(incomplmode) = ncomp;
% smodenoncp = size(datnoncp);
% smodeynoncp = smodenoncp;
% smodeynoncp(incomplmode) = ncomp;

% % unfold datnoncp
% reshapesize    = [smodenoncp(1) prod(smodenoncp(setdiff(1:nmode,1)))];
% datnoncpunfold = reshape(datnoncp, reshapesize);
...
disp([dispprefix 'incomplete mode bigger than combined utility modes: using Cholesky decomposition to speed up computation'])
datorig = dat;
% create datforq
...
while (abs((ssqres - prevssqres) / prevssqres) > convcrit) && (iter < niter) % needs eps failsafe
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % Calculate P by using Q, and assemble Y based on Xk*Pk
  % calculate Pk by Qk
  ...
  %       % first, permute and unfold so estimmode is at the front, and can be indexed
  %       permorder   = [estimmode setdiff(modeindex,estimmode)];
  %       reshapesize = [smodenoncp(estimmode) prod(smodenoncp(setdiff(modeindex,estimmode)))];
  %       currdatforq = reshape(permute(datnoncp,permorder),reshapesize);
  %       % select part of dat for the current estimval, and reshape again to full array
  %       currdatforq = currdatforq(iestimval,:);
  %       reshapesize = smodenoncp(setdiff(modeindex,estimmode));
  %       currdatforq = reshape(currdatforq,reshapesize);
  %       % now unfold data from the perspective of incomplmode
  %       permorder   = [incomplmode setdiff(find(specmodes~=3),incomplmode)];
  %       reshapesize = [smodenoncp(incomplmode) prod(smodenoncp(setdiff(find(specmodes~=3),incomplmode)))];
  ...
  %       currdatforq = currdatforq(1:smodeincomplnoncp(iestimval),:); % NaNs should be the same over all utility modes
  ...
  %       modelforq   = calcmodel_parafac(compforq); % this can be done using regular parafac model formulation
  %       % reshape modelforq into array
  %       reshapesize = smodeynoncp(specmodes~=3);
  %       modelforq   = reshape(modelforq,reshapesize);
  %       % permute so incomplete dimension is first, and unfold
  %       permorder   = [incomplmode setdiff(find(specmodes~=3),incomplmode)];
  %       reshapesize = [ncomp prod(smodenoncp(setdiff(find(specmodes~=3),incomplmode)))];
  %       modelforq   = reshape(permute(modelforq,permorder),reshapesize);
  %       % transpose (regular transpose, as this is data-handling related)
  %       modelforq   = modelforq.';
  %
  ...
  %       P{iestimval} = Pk;
  %     end
  %     model     = calcmodel_parafac2(comp,P,utilitymodes,estimmode,smodenoncp,smodeincomplnoncp); % subfunction for calculating model
  %     ssqres    = nansum(nansum(abs(datnoncpunfold - model).^2));
  %     qrexpvar  = expvar;
  %     expvar    = 100 - ((ssqres / ssqdat) * 100);
...
% set stuff
nmodes    = length(comp); % number of modes
